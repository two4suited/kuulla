...
public class BrowseAndPlaybackTests(WebAppFixture fixture)
{
    ...
    public async Task Search_OpenShow_OpenEpisode_AndPlayAudio()
    {
        ...
        // Longer timeout than the default 30s: the first load of a show's episode list fetches
        // and parses the real RSS feed server-side (Kuulla.Api's EpisodeService), which can be
        // slower than a typical UI action.
        ...
        // The native <audio controls> play button sits at the far left of the control bar, and
        // native controls are a closed shadow root Playwright can't address by role/label — a
        // real click on it (rather than calling el.play() from script) is what preserves the
        // user-gesture that Chromium's autoplay policy requires. Position is relative to the
        // element itself, so this stays centered regardless of the control bar's rendered height.
