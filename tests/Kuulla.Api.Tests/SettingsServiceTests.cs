...
public class SettingsServiceTests
{
    ...
    [Fact]
    public async Task UpdateVoiceBoostAsync_IncrementsVersionOfExistingDocument()
    {
        var existing = new UserSettings(
            UserId, UnlistenedEpisodeCount.Five, Version: 3,
            AutoArchiveRule.Never, VoiceBoost: false);
        _settingsContainer
            .Setup(c => c.ReadItemAsync<UserSettings>(UserId, It.IsAny<PartitionKey>(), null, default))
            .ReturnsAsync(CosmosTestHelpers.ItemResponse(existing));
        _settingsContainer
        ...
        var result = await _sut.UpdateVoiceBoostAsync(UserId, true, CancellationToken.None);

        Assert.True(result.VoiceBoost);
        Assert.Equal(4, result.Version);
    }

    [Fact]
    public async Task UpdateShowVoiceBoostAsync_ClearsOverrideWhenValueIsNull()
    {
        const string showId = "show-1";
        var id = ShowSettings.BuildId(UserId, showId);
        var existing = new ShowSettings(id, UserId, showId, UnlistenedEpisodeCount.Ten, Version: 2, VoiceBoost: true);
        _settingsContainer
            .Setup(c => c.ReadItemAsync<ShowSettings>(id, It.IsAny<PartitionKey>(), null, default))
            .ReturnsAsync(CosmosTestHelpers.ItemResponse(existing));
        _settingsContainer
        ...
        var result = await _sut.UpdateShowVoiceBoostAsync(UserId, showId, null, CancellationToken.None);

        Assert.Null(result.VoiceBoost);
    }

    [Fact]
    public async Task GetEffectiveVoiceBoostAsync_ReturnsShowOverrideWhenSet()
    {
        const string showId = "show-1";
        var showSettingsId = ShowSettings.BuildId(UserId, showId);
        var showSettings = new ShowSettings(showSettingsId, UserId, showId, null, Version: 2, VoiceBoost: true);
        _settingsContainer
            .Setup(c => c.ReadItemAsync<ShowSettings>(showSettingsId, It.IsAny<PartitionKey>(), null, default))
            .ReturnsAsync(CosmosTestHelpers.ItemResponse(showSettings));

        var result = await _sut.GetEffectiveVoiceBoostAsync(UserId, showId, CancellationToken.None);
    ...
    [Fact]
    public async Task GetEffectiveVoiceBoostAsync_FallsBackToUserSettingsWhenNoOverride()
    {
        const string showId = "show-1";
        var showSettingsId = ShowSettings.BuildId(UserId, showId);
        _settingsContainer
            ...
            .ThrowsAsync(CosmosTestHelpers.NotFound());
        var userSettings = new UserSettings(
            UserId, UnlistenedEpisodeCount.Five, Version: 1, AutoArchiveRule.Never, VoiceBoost: true);
        _settingsContainer
            .Setup(c => c.ReadItemAsync<UserSettings>(UserId, It.IsAny<PartitionKey>(), null, default))
            .ReturnsAsync(CosmosTestHelpers.ItemResponse(userSettings));
